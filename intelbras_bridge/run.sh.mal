...
# --- FASE 2: CREAR CONFIG.CFG Y VERIFICAR CONEXIÓN ---
bashio::log.info "Creating config.cfg file..."
cat > ./config.cfg <<EOF
[geral]
addr = 0.0.0.0
...
bashio::log.info "Testing connection and getting initial status from alarm panel..."
OUTPUT=$(./comandar "${ALARM_IP}" "${ALARM_PORT}" "${ALARM_PASS}" "${PASS_LEN}" status)
EXIT_CODE=$?

if [ ${EXIT_CODE} -ne 0 ]; then
  bashio::log.fatal "Connection to alarm panel failed. Please check IP, Port, and Password."
  exit 1
fi
...
function poll_and_update_status() {
    ...
    mosquitto_pub "${MQTT_OPTS[@]}" -r -t "${AVAILABILITY_TOPIC}" -m "online"
    # ... (el resto del parseo de la función es igual)
}
function process_event_stream() {
    while read -r event_line; do
        # ... (el código de la función es igual)
    done
}
