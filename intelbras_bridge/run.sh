...
# Verificar que estamos en /alarme-intelbras
...
if [[ ! -x ./comandar ]]; then chmod +x ./comandar; fi
if [[ ! -x ./receptorip ]]; then chmod +x ./receptorip; fi
...
log_with_timestamp "Executing: ./comandar ${ALARM_IP} ${ALARM_PORT} [HIDDEN_PASS] ${PASS_LEN} status"
...
log_with_timestamp "Alarm status output (first 3 lines): $(head -n 3 status_test.log)"

# --- MQTT DISPONIBILIDAD INICIAL ---
